...
| 영역 | 기능 |
|------|------|
| **검색** | Vector + Keyword Hybrid → RRF → 한국어 리랭킹 → Top-5 |
| **탐색** | 권한별 **사내 문서 트리** 사이드바 (클릭 → 질문) |
...
| **UI** | react-markdown + GFM, 출처 뱃지, 👍/👎 피드백, 인앱 가이드 |
...
4. **생성** — Top-5를 System Prompt에 주입, `[출처: filename.md]` 인용·**한국어 답변** 강제
5. **품질 측정** — `data/eval-queries.json` 8문항, `npm run eval:search` 또는 Admin `/api/admin/metrics`
...
레이아웃: **왼쪽 문서 트리** + **가운데 채팅** (데스크톱). 모바일은 헤더 **「문서」** 버튼으로 트리 드로어.

- NextAuth 세션 기반 — Role은 **로그인 계정에서 자동** (UI 드롭다운 없음)
- **사내 문서 트리** — `vault/` 폴더 구조, RBAC로 열람 가능 문서만 표시 (`GET /api/documents/tree`)
- 문서 클릭 시 해당 문서 내용으로 **자동 질문** 전송
...
초과 시 `429` + `Retry-After` 헤더 (인메모리, `src/lib/rate-limit.ts`)
...
3. 왼쪽 **사내 문서 트리**에서 `연차 휴가 규정` 클릭 또는 "우리 회사 휴가 규정 알려줘" 질의
4. `[출처: 연차휴가규정.md]` 확인
...
│   │   ├── document-tree.tsx     # 사내 문서 트리 사이드바
...
│   │   ├── search/               # reranker, korean-query, metrics
